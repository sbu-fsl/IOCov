...
int pan_argc;
char **pan_argv;
void (*spin_after_argparse)(int argc, char **argv);
int absfs_hash_method = 2;
#ifdef LOOPSTATE
double cnt_loops;
#endif
...
#endif
	if (c_revert_before)
		c_revert_before(p_t_r);
	if (c_revert_after)
		c_revert_after(p_t_r);
}
#endif
void
globinit(void)
...
{	int j = 0, h = now._nr_pr;
	...
	case 1:	/* driver */
		...
		reached1[1]=1;
		/* params: */
		((P1 *)pptr(h))->nproc = par0;
		/* locals: */
		((P1 *)pptr(h))->i = 0;
#ifdef VAR_RANGES
		logval("driver:nproc", ((P1 *)pptr(h))->nproc);
		logval("driver:i", ((P1 *)pptr(h))->i);
#endif
#ifdef HAS_CODE
		locinit1(h);
#endif
	...
	case 0:	/* worker */
		...
		/* params: */
		/* locals: */
		((P0 *)pptr(h))->open_flags = 0;
#ifdef VAR_RANGES
		logval("worker:open_flags", ((P0 *)pptr(h))->open_flags);
#endif
#ifdef HAS_CODE
		locinit0(h);
#endif
	...
	{	switch (argv[1][1]) {
		...
		case 'H':
         ...
         exit(1);
				...
				argv++;
				argc--;
				if (strncmp(argv[1], "md5", strlen("md5")) == 0) {
					absfs_hash_method = 2;
				} else if (strncmp(argv[1], "xxh64", strlen("xxh64")) == 0) {
					absfs_hash_method = 1;
				} else if (strncmp(argv[1], "xxh128", strlen("xxh128")) == 0) {
					absfs_hash_method = 0;
				} else if (strncmp(argv[1], "crc32", strlen("crc32")) == 0) {
					absfs_hash_method = 3;
				} else {
					fprintf(stderr, "-H must follow a supported abstraction hash method: md5, xxh64 or xxh128, but got '%s'.", argv[1]);
					exit(1);
				}
...
{	/* int i; */
	...
	case 2:
		...
		break;
	case 1:
		printf("local vars proc %d (driver):\n", pid);
	printf("	int    nproc:	%d\n", ((P1 *)pptr(pid))->nproc);
	printf("	int    i:	%d\n", ((P1 *)pptr(pid))->i);
		break;
	case 0:
		printf("local vars proc %d (worker):\n", pid);
	printf("	int    open_flags:	%d\n", ((P0 *)pptr(pid))->open_flags);
		break;
	}
}
void
